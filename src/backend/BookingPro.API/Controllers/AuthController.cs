...
namespace BookingPro.API.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> ForgotPassword([FromBody] ForgotPasswordDto dto)
        {
            ...
            {
                // Build the reset link on the same host the request came from. The app
                // SPA (with the /reset-password route) is served on both the apex and
                // the tenant subdomains, and the token itself carries the tenant, so the
                // reset works regardless of host.
